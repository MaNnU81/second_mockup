...
var ScrollableService = class _ScrollableService {
  ...
  subscribe(callback) {
    ...
    this._zone.runOutsideAngular(() => {
      const observables = parents2.map((p) => (0, import_rxjs.fromEvent)(p, "scroll").pipe((0, import_operators.auditTime)(FRAME_DURATION)));
      const subscriber = (e) => {
...
var KendoButtonService = class _KendoButtonService {
  buttonClicked = new import_rxjs2.Subject();
...
var ButtonGroupComponent = class _ButtonGroupComponent {
  ...
  ngOnInit() {
    ...
    this.subs.add((0, import_rxjs2.fromEvent)(this.element.nativeElement, "focusout").pipe((0, import_operators2.filter)((event) => this.navigable && event.relatedTarget && event.relatedTarget.parentNode !== this.element.nativeElement)).subscribe(() => {
      this.defocus(this.buttons.toArray());
      this.currentTabIndex = this.tabIndex;
    }));
  }
  ...
  handleSubs(eventName, predicate, handler) {
    this.subs.add((0, import_rxjs2.fromEvent)(this.element.nativeElement, eventName).pipe((0, import_operators2.filter)(predicate)).subscribe(handler));
  }
  focusHandler = () => {
    this.currentTabIndex = -1;
    this.defocus(this.buttons.toArray());
...
var ListButton = class _ListButton extends MultiTabStop {
  ...
  direction;
  popupRef;
  popupSubs = new import_rxjs2.Subscription();
  button;
  buttonList;
  popupTemplate;
  container;
  ...
  subscribeComponentBlurredEvent() {
    this._zone.runOutsideAngular(() => {
      this.subs.add(this.navigationService.tab.pipe((0, import_operators2.filter)(() => this._isFocused), (0, import_operators2.tap)(() => this.focusButton())).subscribe(this.handleTab.bind(this)));
      this.subs.add((0, import_rxjs2.fromEvent)(document, "click").pipe((0, import_operators2.filter)((event) => !this.wrapperContains(event.target)), (0, import_operators2.filter)(() => this._isFocused)).subscribe(() => this._zone.run(() => this.blurWrapper())));
    });
  }
  subscribeNavigationEvents() {
    ...
    this.subs.add((0, import_rxjs2.merge)(this.navigationService.close, this.navigationService.esc).subscribe(this.onNavigationClose.bind(this)));
  }
  /**
...
var FloatingActionButtonComponent = class _FloatingActionButtonComponent {
  ...
  subscribeNavigationEvents() {
    ...
    this.subscriptions.add((0, import_rxjs2.merge)(this.navigationService.close, this.navigationService.esc).subscribe(this.onNavigationClose.bind(this)));
  }
  onArrowKeyNavigate({
    index
  }) {
  ...
  closeDial() {
    if (this.dialItemAnimation && this.isValidAnimation()) {
      ...
      this.animationEnd.pipe((0, import_operators2.take)(1)).subscribe(() => this.closePopup());
    } else {
      this.closePopup();
    }
...
var SpeechToTextButtonComponent = class _SpeechToTextButtonComponent {
  ...
  onClick() {
    if (this.isWebSpeechSupported && this.integrationMode === "webSpeech") {
      ...
      let asyncFactory = () => (0, import_rxjs2.of)(null);
      this.ngZone.run(() => {
        if (this.isListening) {
        ...
        observable.pipe((0, import_operators2.take)(1)).subscribe(() => this.isListening = !this.isListening);
      });
    }
  }
  /**
  ...
  toObservable(input) {
    return input instanceof import_rxjs2.Observable ? input : (0, import_rxjs2.from)(input);
  }
  handleThemeColor(value, prevFillMode, fillMode) {
    const elem = this.element;
    const removeFillMode = prevFillMode || this.fillMode;
...
//# sourceMappingURL=chunk-VBD3CQUY.js.map
