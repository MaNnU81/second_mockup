...
var AdaptiveService = class _AdaptiveService {
   ...
   * @hidden
   */
  changes = new import_rxjs.BehaviorSubject(this.adaptiveSettings || {
    small: 500,
    medium: 700
  });
  /**
   ...
   * @hidden
   */
  sizeChanges = new import_rxjs.BehaviorSubject(this.size);
  ...
  constructor(_adaptiveSettings, adaptiveSettingsService, zone) {
    ...
    if (adaptiveSettingsService) {
      this.subs.add(adaptiveSettingsService.changes.pipe((0, import_operators.map)((adaptiveSettings) => isPresent(adaptiveSettings) ? adaptiveSettings : this._adaptiveSettings), (0, import_operators.tap)((adaptiveSettings) => this._adaptiveSettings = adaptiveSettings)).subscribe((adaptiveSettings) => this.changes.next(adaptiveSettings)));
    }
    ...
    if (isDocumentAvailable()) {
      this.zone.runOutsideAngular(() => {
        this.subs.add((0, import_rxjs.fromEvent)(window, "resize").pipe((0, import_operators.tap)(() => !this.previousSize && (this.previousSize = this.size)), (0, import_operators.filter)(() => this.previousSize !== this.size)).subscribe(() => {
...
var ActionSheetComponent = class _ActionSheetComponent {
  ...
  dynamicRTLSubscription;
  rtl = false;
  domSubs = new import_rxjs2.Subscription();
  player;
...
var BreadCrumbListComponent = class _BreadCrumbListComponent {
  ...
  constructor(el, zone) {
    ...
    this.zone.runOutsideAngular(() => {
      const click$ = (0, import_rxjs2.fromEvent)(element, "click");
      const enterKey$ = (0, import_rxjs2.fromEvent)(element, "keydown").pipe((0, import_operators2.filter)((ev) => ev.code === Keys.Enter || ev.code === Keys.NumpadEnter));
      this.domEventsSubscription = (0, import_rxjs2.merge)(click$, enterKey$).pipe(
        (0, import_operators2.map)((ev) => ev.target),
        (0, import_operators2.filter)((e) => !e.classList.contains("k-breadcrumb-delimiter-icon")),
        // do not trigger handler when a separator is clicked
        (0, import_operators2.map)((e) => this.getItemIndex(e)),
        (0, import_operators2.filter)(isPresent3),
        (0, import_operators2.map)((i) => parseInt(i, 10)),
        (0, import_operators2.map)((i) => this.items[i]),
        (0, import_operators2.filter)((item) => !item.data.disabled && !item.context.isLast),
        (0, import_operators2.map)((item) => item.data)
      ).subscribe((el2) => {
...
var BreadCrumbComponent = class _BreadCrumbComponent {
  ...
  _collapseMode = "auto";
  _size = DEFAULT_SIZE;
  updateItems = new import_rxjs2.ReplaySubject();
  afterViewInit = new import_rxjs2.Subject();
  subscriptions = new import_rxjs2.Subscription();
  direction = "ltr";
  constructor(localization, el, cdr, zone, renderer) {
    ...
    this.renderer = renderer;
    validatePackage(packageMetadata2);
    const updateItems$ = this.updateItems.asObservable().pipe((0, import_operators2.startWith)([]));
    this.direction = localization.rtl ? "rtl" : "ltr";
    this.itemsData$ = updateItems$.pipe((0, import_operators2.map)((items) => items.filter(Boolean)), (0, import_operators2.map)((items) => items.map((item, index, collection) => ({
      context: {
        collapsed: false,
    ...
    this.firstItem$ = updateItems$.pipe((0, import_operators2.map)((items) => {
      if (items.length > 0) {
        return [{
          context: {
            collapsed: false,
  ...
  attachResizeHandler() {
    this.subscriptions.add((0, import_rxjs2.merge)(this.resizeSensor.resize, this.itemsData$, this.afterViewInit.asObservable()).pipe((0, import_operators2.filter)(() => this.shouldResize())).subscribe(() => {
      this.resizeHandler();
    }));
  }
...
//# sourceMappingURL=chunk-4DWW4XOS.js.map
