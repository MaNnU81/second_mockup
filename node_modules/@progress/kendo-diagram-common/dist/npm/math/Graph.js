...
exports.DiagramToHyperTreeAdapter = exports.SpringLayout = exports.Graph = exports.Link = exports.Node = void 0;
...
const LayoutBase_1 = require("./../layout/LayoutBase");
const LayoutState_1 = require("./../layout/LayoutState");
/**
...
class Node {
    constructor(id, shape) {
        ...
        if ((0, utils_1.isDefined)(id)) {
            ...
            this.id = (0, randomId_1.randomId)();
        }
        if ((0, utils_1.isDefined)(shape)) {
     ...
     */
    isIsolated() {
        return (0, utils_1.isEmpty)(this.links);
    }
    /**
     ...
     */
    bounds(r) {
        if (!(0, utils_1.isDefined)(r)) {
            return new Shapes_1.Rect(this.x, this.y, this.width, this.height);
        }
        this.x = r.x;
        this.y = r.y;
        this.width = r.width;
     ...
     */
    isLinkedTo(node) {
        return (0, utils_1.getAny)(this.links, (link) => {
    ...
    clone() {
        const copy = new Node();
        if ((0, utils_1.isDefined)(this.weight)) {
            copy.weight = this.weight;
        }
        if ((0, utils_1.isDefined)(this.balance)) {
            copy.balance = this.balance;
        }
        if ((0, utils_1.isDefined)(this.owner)) {
    ...
    removeLink(link) {
        if (link.source === this) {
            (0, utils_1.remove)(this.links, link);
            (0, utils_1.remove)(this.outgoing, link);
            link.source = null;
        }
        if (link.target === this) {
            (0, utils_1.remove)(this.links, link);
            (0, utils_1.remove)(this.incoming, link);
            link.target = null;
        }
    }
    /**
     ...
     */
    hasLinkTo(node) {
        return (0, utils_1.getAny)(this.outgoing, function (link) {
     ...
     */
    incidentWith(link) {
        return (0, utils_1.contains)(this.links, link);
    }
    /**
     * Returns the links between this node and the given one.
     */
    getLinksWith(node) {
        return (0, utils_1.all)(this.links, function (link) {
            return link.getComplement(this) === node;
        }, this);
    }
    /**
    ...
    getNeighbors() {
        const neighbors = [];
        (0, utils_1.forEach)(this.incoming, function (e) {
            neighbors.push(e.getComplement(this));
        }, this);
        (0, utils_1.forEach)(this.outgoing, function (e) {
            neighbors.push(e.getComplement(this));
        }, this);
        return neighbors;
    }
...
class Link {
    constructor(source, target, id, connection) {
        ...
        if ((0, utils_1.isDefined)(connection)) {
     ...
     */
    adjacentTo(link) {
        return (0, utils_1.contains)(this.source.links, link) || (0, utils_1.contains)(this.target.links, link);
    }
    /**
     * Changes the source-node of this link.
     */
    changeSource(node) {
        (0, utils_1.remove)(this.source.links, this);
        (0, utils_1.remove)(this.source.outgoing, this);
     ...
     */
    changeTarget(node) {
        (0, utils_1.remove)(this.target.links, this);
        (0, utils_1.remove)(this.target.incoming, this);
        node.links.push(this);
    ...
    reverse() {
        ...
        (0, utils_1.remove)(oldSource.outgoing, this);
        this.source.outgoing.push(this);
        this.target = oldSource;
        (0, utils_1.remove)(oldTarget.incoming, this);
        this.target.incoming.push(this);
        return this;
    }
    /**
...
class Graph {
    constructor(idOrDiagram) {
         ...
         */
        this.nodes = [];
        this._nodeMap = new Dictionary_1.Dictionary();
        /**
         ...
         * @type {Rect}
         */
        this.bounds = new Shapes_1.Rect();
        // keeps track whether the children & parents have been created
        this._hasCachedRelationships = false;
        this.type = 'Graph';
        this.componentIndex = 0;
        if ((0, utils_1.isDefined)(idOrDiagram)) {
            if ((0, utils_1.isString)(idOrDiagram)) {
                this.id = idOrDiagram;
            }
            else {
    ...
    cacheRelationships(forceRebuild) {
        if ((0, utils_1.isUndefined)(forceRebuild)) {
            forceRebuild = false;
        }
    ...
    assignLevels(startNode, offset, visited) {
        ...
        if ((0, utils_1.isUndefined)(offset)) {
            offset = 0;
        }
        // if not done before, cache the parents and children
        this.cacheRelationships();
        if ((0, utils_1.isUndefined)(visited)) {
            visited = new Dictionary_1.Dictionary();
            (0, utils_1.forEach)(this.nodes, function (n) {
                visited.add(n, false);
            });
        }
    ...
    root(value) {
        if ((0, utils_1.isUndefined)(value)) {
            if (!this._root) {
                ...
                const found = (0, utils_1.first)(this.nodes, function (n) {
                    return n.incoming.length === 0;
                });
                if (found) {
                    return found;
                }
                return (0, utils_1.first)(this.nodes);
            }
            else {
                return this._root;
            }
    ...
    getConnectedComponents() {
        this.componentIndex = 0;
        this.setItemIndices();
        const componentId = (0, utils_1.initArray)(this.nodes.length, -1);
    ...
    _collectConnectedNodes(setIds, nodeIndex) {
        ...
        (0, utils_1.forEach)(node.links, function (link) {
    ...
    getSpanningTree(root) {
        ...
        const map = new Dictionary_1.Dictionary();
        let source, target;
        ...
        while (remaining.length > 0) {
            ...
            for (let ni = 0; ni < next.links.length; ni++) {
                ...
                if ((0, utils_1.contains)(visited, cn)) {
                    continue;
                }
                ...
                if (!(0, utils_1.contains)(remaining, cn)) {
                    remaining.push(cn);
                }
                if (!(0, utils_1.contains)(visited, cn)) {
                    visited.push(cn);
        ...
        (0, utils_1.forEach)(tree.nodes, function (node) {
    ...
    takeRandomNode(excludedNodes, incidenceLessThan) {
        if ((0, utils_1.isUndefined)(excludedNodes)) {
            excludedNodes = [];
        }
        if ((0, utils_1.isUndefined)(incidenceLessThan)) {
            incidenceLessThan = 4;
        }
        ...
        if (this.nodes.length === 1) {
            return (0, utils_1.contains)(excludedNodes, this.nodes[0]) ? null : this.nodes[0];
        }
        const pool = this.nodes.filter(function (node) {
            return !(0, utils_1.contains)(excludedNodes, node) && node.degree() <= incidenceLessThan;
        });
        if ((0, utils_1.isEmpty)(pool)) {
            return null;
        }
        return pool[(0, utils_1.randomInteger)(0, pool.length)];
    }
    /**
     * Returns whether this is an empty graph.
     */
    isEmpty() {
        return (0, utils_1.isEmpty)(this.nodes);
    }
    /**
    ...
    isHealthy() {
        return (0, utils_1.all)(this.links, function (link) {
            return (0, utils_1.contains)(this.nodes, link.source) && (0, utils_1.contains)(this.nodes, link.target);
        }, this);
    }
    /**
     * Gets the parents of this node, defined as the adjacent nodes with a link from the adjacent node to this one.
    ...
    addLink(sourceOrLink, target, owner) {
        ...
        if ((0, utils_1.isUndefined)(foundSource)) {
            foundSource = this.addNode(sourceOrLink);
        }
        let foundTarget = this.getNode(target);
        if ((0, utils_1.isUndefined)(foundTarget)) {
            foundTarget = this.addNode(target);
        }
        const newLink = new Link(foundSource, foundTarget);
        if ((0, utils_1.isDefined)(owner)) {
            newLink.owner = owner;
        }
     ...
     */
    hasLink(linkOrId) {
        if ((0, utils_1.isString)(linkOrId)) {
            return (0, utils_1.getAny)(this.links, function (link) {
                return link.id === linkOrId;
            });
        }
        if (linkOrId.type === 'Link') {
            return (0, utils_1.contains)(this.links, linkOrId);
        }
    ...
    _removeNode(node) {
        (0, utils_1.remove)(this.nodes, node);
    ...
    removeNode(nodeOrId) {
        let n = nodeOrId;
        if ((0, utils_1.isString)(nodeOrId)) {
            n = this.getNode(nodeOrId);
        }
        if ((0, utils_1.isDefined)(n)) {
            const links = n.links;
            n.links = [];
    ...
    removeLink(link) {
        /*    if (!this.links.contains(link)) {
            ...
            */
        (0, utils_1.remove)(this.links, link);
        (0, utils_1.remove)(link.source.outgoing, link);
        (0, utils_1.remove)(link.source.links, link);
        (0, utils_1.remove)(link.target.incoming, link);
        (0, utils_1.remove)(link.target.links, link);
    }
    /**
    ...
    addNode(nodeOrId, layoutRect, owner) {
        let newNode = null;
        if (!(0, utils_1.isDefined)(nodeOrId)) {
        ...
        if ((0, utils_1.isDefined)(layoutRect)) {
    ...
    addNodeAndOutgoings(node) {
        ...
        node.outgoing = [];
        (0, utils_1.forEach)(newLinks, function (link) {
    ...
    clone(saveMapping) {
        ...
        const save = (0, utils_1.isDefined)(saveMapping) && saveMapping === true;
        if (save) {
            copy.nodeMap = new Dictionary_1.Dictionary();
            copy.linkMap = new Dictionary_1.Dictionary();
        }
        // we need a map even if the saveMapping is not set
        const map = new Dictionary_1.Dictionary();
        (0, utils_1.forEach)(this.nodes, function (nOriginal) {
        ...
        (0, utils_1.forEach)(this.links, function (linkOriginal) {
    ...
    depthFirstTraversal(startNode, action) {
        if ((0, utils_1.isUndefined)(startNode)) {
            throw new Error('You need to supply a starting node.');
        }
        if ((0, utils_1.isUndefined)(action)) {
    ...
    _dftIterator(node, action, visited) {
        ...
        for (let i = 0, len = children.length; i < len; i++) {
            const child = children[i];
            if ((0, utils_1.contains)(visited, child)) {
                continue;
            }
    ...
    breadthFirstTraversal(startNode, action) {
        ...
        while (queue.length > 0) {
            ...
            for (let i = 0, len = children.length; i < len; i++) {
                const child = children[i];
                if ((0, utils_1.contains)(visited, child) || queue.contains(child)) {
                    continue;
                }
                queue.enqueue(child);
            }
    ...
    _stronglyConnectedComponents(excludeSingleItems, node, indices, lowLinks, connected, stack, index) {
        ...
        for (let i = 0, len = children.length; i < len; i++) {
            ...
            else if ((0, utils_1.contains)(stack, next)) {
    ...
    findCycles(excludeSingleItems) {
        if ((0, utils_1.isUndefined)(excludeSingleItems)) {
            excludeSingleItems = true;
        }
        const indices = new Dictionary_1.Dictionary();
        const lowLinks = new Dictionary_1.Dictionary();
        const connected = [];
        const stack = [];
        for (let i = 0, len = this.nodes.length; i < len; i++) {
            const node = this.nodes[i];
     ...
     */
    isAcyclic() {
        return (0, utils_1.isEmpty)(this.findCycles());
    }
    /**
    ...
    isSubGraph(other) {
        ...
        return (0, utils_1.all)(otherArray, function (s) {
            return (0, utils_1.contains)(thisArray, s);
        });
    }
    /**
    ...
    makeAcyclic() {
        ...
        const intensityCatalog = new Dictionary_1.Dictionary();
        /**
        ...
        (0, utils_1.forEach)(copy.nodes, function (v) {
            ...
            if (intensityCatalog.containsKey(2 - N)) {
                ...
                while (targets.length > 0) {
                    ...
                    for (let li = 0; li < target.links.length; li++) {
                        ...
                        (0, utils_1.remove)(intensityCatalog.get(intensity), source);
            ...
            if (intensityCatalog.containsKey(N - 2)) {
                ...
                while (sources.length > 0) {
                    ...
                    for (let si = 0; si < source.links.length; si++) {
                        ...
                        (0, utils_1.remove)(intensityCatalog.get(intensity), target);
                ...
                for (let k = N - 3; k > 2 - N; k--) {
                    if (intensityCatalog.containsKey(k) &&
                        ...
                        for (let ri = 0; ri < v.links.length; ri++) {
                            ...
                            (0, utils_1.remove)(intensityCatalog.get(intensity), u);
                            u.removeLink(ril);
        ...
        const vertexOrder = new Dictionary_1.Dictionary();
        for (let kk = 0; kk < this.nodes.length; kk++) {
            vertexOrder.set(copy.nodeMap.get(sourceStack[kk]), kk);
        }
        const reversedEdges = [];
        (0, utils_1.forEach)(this.links, function (link) {
...
Graph.Predefined = {
     ...
     */
    BinaryTree(levels) {
        if ((0, utils_1.isUndefined)(levels)) {
            levels = 5;
        }
     ...
     */
    Linear(length) {
        if ((0, utils_1.isUndefined)(length)) {
            length = 10;
        }
...
Graph.Utils = {
    ...
    parse(graphString) {
        ...
        for (let i = 0, len = parts.length; i < len; i++) {
            const part = parts[i];
            if ((0, utils_1.isString)(part)) { // link spec
                if (part.indexOf('->') < 0) {
            ...
            if ((0, utils_1.isObject)(part)) {
                if (!previousLink) {
                    throw new Error('Specification found before Link definition.');
                }
                (0, deep_extend_1.deepExtend)(previousLink, part);
            }
        }
        return graph;
    },
    ...
    linearize(graph, addIds) {
        ...
        if ((0, utils_1.isUndefined)(addIds)) {
            addIds = false;
        }
        const lin = [];
    ...
    _addShape(kendoDiagram, p, id, shapeDefaults) {
        if ((0, utils_1.isUndefined)(p)) {
            p = new Shapes_1.Point(0, 0);
        }
        if ((0, utils_1.isUndefined)(id)) {
            id = (0, randomId_1.randomId)();
        }
        shapeDefaults = (0, deep_extend_1.deepExtend)({
            width: 20,
            height: 20,
            id: id,
            radius: 10,
    ...
    createDiagramFromGraph(diagram, graph, doLayout, randomSize) {
        ...
        if ((0, utils_1.isUndefined)(doLayout)) {
            doLayout = true;
        }
        if ((0, utils_1.isUndefined)(randomSize)) {
            randomSize = false;
        }
        ...
        for (let i = 0, len = graph.nodes.length; i < len; i++) {
            ...
            let p = node.position;
            if ((0, utils_1.isUndefined)(p)) {
                if ((0, utils_1.isDefined)(node.x) && (0, utils_1.isDefined)(node.y)) {
                    p = new Shapes_1.Point(node.x, node.y);
                }
                else {
                    p = new Shapes_1.Point((0, utils_1.randomInteger)(10, width - 20), (0, utils_1.randomInteger)(10, height - 20));
                }
            }
            const opt = {};
            if (node.id === '0') {
                ...
                (0, deep_extend_1.deepExtend)(opt, {
        ...
        for (let gli = 0; gli < graph.links.length; gli++) {
            ...
            if ((0, utils_1.isUndefined)(sourceShape)) {
                continue;
            }
            const targetShape = map[link.target.id];
            if ((0, utils_1.isUndefined)(targetShape)) {
                continue;
            }
        ...
        if (doLayout) {
            ...
            for (let shi = 0; shi < graph.nodes.length; shi++) {
                ...
                shape = map[node.id];
                shape.bounds(new Shapes_1.Rect(node.x, node.y, node.width, node.height));
            }
        }
    },
    /**
    ...
    createBalancedTree(levels, siblingsCount) {
        ...
        if ((0, utils_1.isUndefined)(siblingsCount)) {
            siblingsCount = 3;
        }
        const g = new Graph();
    ...
    createBalancedForest(levels, siblingsCount, treeCount) {
        ...
        if ((0, utils_1.isUndefined)(treeCount)) {
            treeCount = 5;
        }
        const g = new Graph();
    ...
    createRandomConnectedGraph(nodeCount, maxIncidence, isTree) {
        /* Swa's Mathematica export of random Bernoulli graphs
           ...
           export [g]
           */
        if ((0, utils_1.isUndefined)(nodeCount)) {
            nodeCount = 40;
        }
        if ((0, utils_1.isUndefined)(maxIncidence)) {
            maxIncidence = 4;
        }
        if ((0, utils_1.isUndefined)(isTree)) {
            isTree = false;
        }
        const g = new Graph();
        let counter = -1;
        ...
        if (nodeCount > 1) {
            ...
            if (!isTree && nodeCount > 1) {
                const randomAdditions = (0, utils_1.randomInteger)(1, nodeCount);
 ...
 * @type {*}
 */
class SpringLayout extends LayoutBase_1.LayoutBase {
    constructor(diagram) {
        super();
        if ((0, utils_1.isUndefined)(diagram)) {
    ...
    _repulsion(n) {
        n.dx = 0;
        n.dy = 0;
        (0, utils_1.forEach)(this.graph.nodes, function (m) {
            if (m === n) {
                return;
            }
    ...
    _expectedBounds() {
        ...
        const size = (0, utils_1.fold)(this.graph.nodes, function (s, node) {
...
exports.SpringLayout = SpringLayout;
/**
...
class DiagramToHyperTreeAdapter {
    constructor(diagram) {
         ...
         * @type {Dictionary}
         */
        this.shapeMap = new Dictionary_1.Dictionary();
        /**
        ...
        this.edgeMap = new Dictionary_1.Dictionary();
        /**
         ...
         * @type {Dictionary}
         */
        this.hyperMap = new Dictionary_1.Dictionary();
        /**
     ...
     */
    convert(options) {
        if ((0, utils_1.isUndefined)(this.diagram)) {
            throw new Error('No diagram to convert.');
        }
        this.options = (0, deep_extend_1.deepExtend)({
        ...
        this.finalNodes = new Dictionary_1.Dictionary(this.nodes);
        this.finalLinks = new Dictionary_1.Dictionary(this.edges);
     ...
     */
    getEdge(a, b) {
        return (0, utils_1.first)(a.links, function (link) {
    ...
    clear() {
        ...
        this.hyperMap = (!this.options.ignoreContainers && this.options.layoutContainerChildren) ? new Dictionary_1.Dictionary() : null;
        this.nodeMap = new Dictionary_1.Dictionary();
        this.shapeMap = new Dictionary_1.Dictionary();
        this.nodes = [];
        this.edges = [];
    ...
    isDescendantOf(scope, a) {
        ...
        if ((0, utils_1.contains)(scope.children, a)) {
            return true;
        }
        const containers = [];
    ...
    leastCommonAncestor(a, b) {
        ...
        let found = null;
        if ((0, utils_1.isEmpty)(al) || (0, utils_1.isEmpty)(bl)) {
    ...
    _renormalizeConnections() {
        ...
        for (let i = 0, len = this.diagram.connections.length; i < len; i++) {
            ...
            if ((0, utils_1.contains)(this.ignoredShapes, source) && !this.shapeMap.containsKey(source)) {
                this.ignoredConnections.push(conn);
                continue;
            }
            if ((0, utils_1.contains)(this.ignoredShapes, sink) && !this.shapeMap.containsKey(sink)) {
                this.ignoredConnections.push(conn);
                continue;
            }
    ...
    areConnectedAlready(n, m) {
        return (0, utils_1.getAny)(this.edges, function (l) {
            return l.source === n && l.target === m || l.source === m && l.target === n;
        });
    }
}
exports.DiagramToHyperTreeAdapter = DiagramToHyperTreeAdapter;
